...
from tinygrad.llm.kernels.nv import nv_custom_kernels_supported, nv_rmsnorm, nv_add_rmsnorm, nv_normalize, nv_argmax
  ...
  def _feed_forward(self, x:Tensor) -> Tensor:
    if hasattr(self, 'ffn_gate_exps'):
      ...
      if hasattr(self, 'ffn_gate_shexp'):
        ...
        out = out + shexp
      return out
    # Fused SiLU * linear up-projection without contiguous barrier
...
def _q8_0_quantize_kv(x: Tensor) -> tuple[Tensor, Tensor]:
  B, H, T, D = x.shape
  g = x.cast(dtypes.float32).reshape(B, H, T, D // Q8_GROUP_SIZE, Q8_GROUP_SIZE)
  d = (g.abs().max(-1, keepdim=True) / 127.0).maximum(1e-12)
  q = (g / d).round().clamp(-127, 127)
  return q.reshape(B, H, T, D), d.squeeze(-1).cast(dtypes.half)

def _q8_0_dequantize_kv(q: Tensor, d: Tensor, head_dim: int) -> Tensor:
  B, H, T, D = q.shape
  return (q.cast(dtypes.float32).reshape(B, H, T, D // Q8_GROUP_SIZE, Q8_GROUP_SIZE) *
          d.unsqueeze(-1).cast(dtypes.float32)).reshape(B, H, T, D)
...
class Transformer:
  ...
  def forward(self, tokens:Tensor, start_pos:int|UOp, temperature:Tensor) -> Tensor:
    ...
    return nv_argmax(logits / temperature.maximum(1e-12) - (Tensor.rand_like(logits).maximum(1e-12).log().neg()).log(), keepdim=True)
  ...
  def from_gguf(gguf:Tensor|str|pathlib.Path, max_context:int|None=None,
                realize=bool(getenv("REALIZE", 0)), cache_type:str="f16") -> tuple[Transformer, dict]:
